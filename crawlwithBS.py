...
f = csv.writer(open('aloNhaDatData_chungcu_canho.csv', 'w', encoding='utf-8'))
...
for item in pages:
    ...
    for house in house_list:
        ...
        image = house.find(class_='thumbnail').find("img")['src']

        text = house.find(class_='text')

        house_road_width = text.find(class_='road-width')
        if check_data(house_road_width) == True:
            duong_truoc_nha = 'NULL'
        else:
            duong_truoc_nha = house_road_width.contents[0]

        house_floors = text.find(class_='floors')
        ...
        house_parking = text.find(class_='parking')
        if check_data(house_parking) == True :
            cho_de_xe = 'NULL'
        else:
            cho_de_xe = house_parking.contents[0]  

        house_dientich = text.find(class_='ct_dt')
        if check_data(house_dientich) == True :
            dien_tich = 'NULL'
        else:
            dien_tich = house_dientich.contents[1]  + " vuông"

        house_kichthuoc = text.find(class_='ct_kt')
        ...
        house_price = text.find(class_='ct_price')
        if check_data(house_price) == True :
            gia = 'NULL'
        else:
            gia = house_price.contents[1]
        ...
        f.writerow([date, image, duong_truoc_nha, so_tang, so_phong_ngu, cho_de_xe, dien_tich, kich_thuoc, huong_nha,gia, dia_chi])
        # f.writerow(['date', 'image', 'đường trước nhà', 'số tầng', 'số phòng ngủ', 'chỗ để xe', 'diện tích', 'kích thước','hướng nhà', 'giá', 'địa chỉ'])
# f.close()
# print("DONE!!!")